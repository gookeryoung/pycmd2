...
def test_image_processor_with_valid_images(temp_dir):
    # 准备测试图片
...
def test_image_processor_with_no_images(temp_dir, caplog, mocker):
    # Mock is_valid_image函数
    mocker.patch('pycmd2.images.image_to_pdf.is_valid_image', return_value=False)
...
def test_main_function(temp_dir, mocker):
    # 准备测试图片
    img = Image.new('RGB', (100, 100), color='green')
    img.save(temp_dir / 'test.jpg')

    # Mock is_valid_image函数
    mocker.patch('pycmd2.images.image_to_pdf.is_valid_image', side_effect=lambda x: x.suffix.lower() in ('.jpg', '.png'))
...
def test_is_valid_image_check(mocker):
    # Mock实际的is_valid_image函数
    mocker_valid = mocker.patch('pycmd2.images.image_to_pdf.is_valid_image', side_effect=lambda x: x.suffix.lower() in ('.jpg', '.png'))
